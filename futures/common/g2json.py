...
ID_FIELD = '__rowid__'
GOOGLE_USER = 'svc-dsecontent@netflix.com'
GOOGLE_PASSWORD = 'Uj93gMC$X4BMC$4R93g%'
GOOGLE_AGENT = "netflix-rainman-agent"
...
class SpreadsheetAPI(object):
    ...
    def list_spreadsheets(self, client=None):
        """List Spreadsheets.
        :return:
    ...
    def list_worksheets(self, spreadsheet_key, client=None):
        """List Spreadsheets.
        :return:
            A list with information about the spreadsheets available
        """
        ...
        try:
            wks = client.GetWorksheetsFeed(key=spreadsheet_key)
            return map(lambda e: (e.title.text, e.id.text.rsplit('/', 1)[1]), wks.entry)
        except Exception as e:
            if len(e.args) and e.args[0].get("status", None) == 403:
                return None
    ...
    def get_sheet_headers(self, key, sheet_id, client=None):
        ...
        for i, v in enumerate(header_xml.entry):
            if v.cell.inputValue:
                ret.append({"index":i,"friendly":v.cell.inputValue, "programmatic": "".join(clean.findall(v.cell.inputValue.lower().replace('_','')))})

        return ret

    def get_headers(self, key, sheet_id, client=None):
        if not client:
            client = self._get_client()
        ...
        i = 1
        header = client.GetCellsFeed(key, sheet_id, 'R1C1').cell.inputValue
        while header:
            current_headers.append(header)

            i = i + 1
            header = client.GetCellsFeed(key, sheet_id, 'R1C%s' % str(i)).cell.inputValue

        return current_headers


    def add_sheet_headers(self, headers, key, sheet_id, client=None):
        if not client:
            client = self._get_client()

        #gets our current headers
        current_headers = self.get_sheet_headers(key, sheet_id, client)

        for i, header in enumerate(headers):
            client.UpdateCell(row=1, col=len(current_headers) + 1 + i, inputValue=header["name"], key=key, wksht_id=sheet_id)


    def _simplify_headers(self, headers):
        return [header.lower() for header in headers]
        # return [header.lower().replace("_","").replace(" ","") for header in headers]

    def _get_sheet_id(self, key, sheet_name, client=None):
        worksheets = self.list_worksheets(key, client)
        if not worksheets:
            return None

        worksheets_lookup = {name.lower(): id for name, id in worksheets}

        return worksheets_lookup.get(sheet_name.lower(), None) or worksheets[0][1]
...
class Worksheet(object):
    ...
    def _get_row_entries(self, query=None):
        ...
        """
        if not self.entries:
            self.entries = self.gd_client.GetListFeed(query=query, **self.keys).entry
    ...
    def get_rows(self, query=None, order_by=None,
        ...
        rows = [self._row_to_dict(row) for row in self._get_row_entries(query=self.query)]
...
def get_gdoc(key, sheet_name=None, return_json=False):
    API = SpreadsheetAPI(GOOGLE_USER, GOOGLE_PASSWORD, GOOGLE_AGENT)
    client = API._get_client()

    #gets all the rows in the sheet
    sheet_id = API._get_sheet_id(key=key, sheet_name=sheet_name, client=client)
    sheet = API.get_worksheet(key, sheet_id)
    rows = sheet.get_rows()
